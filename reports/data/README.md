...
Files are keyed by run label: SFT `175462d7`, RM baseline `01772c12`, RM higher-capacity `56239d4c`, PPO `e71b6d13`. The SFT `0ac53a30` files are the Llama-3.2-3B comparison run of the SFT report's appendix. DPO files are keyed by the run labels of the DPO report's Section 9, the arm being `75047d16`.
...
| `trainer_state_<label>.json` | the Trainer, at a run's last saved checkpoint | the training-curve figures of the SFT, RM and PPO reports, which `src/analysis/plot_loss_curves.py` and `plot_ppo_curves.py` regenerate from it, and DPO report Sections 7 and 9 (in-training trajectories) |
